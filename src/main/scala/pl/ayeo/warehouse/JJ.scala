...
object JJ  extends App {
  ...
  final case class BodyChanged(postId: String, body: String) extends Event
  ...
  private val eventHandler: (State, Event) => State = { (state, event) =>
    state match {
      ...
      def getBehavior(entityId: String, persistenceId: PersistenceId): Behavior[Command] = {
    ...
    val item = ActorSystem(getBehavior("id", PersistenceId("ulalala", "chra")), "Ulalala")
    item ! AddPost(PostContent("7", "Title", "Content"), item.ignoreRef)
